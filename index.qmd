...
for col in ["2021", "2022"]:
    has_k = emissions[col].astype(str).str.contains("k", na=False)
    values = emissions[col].astype(str).str.replace("k", "", regex=False)
...
emissions_long["emissions"] = pd.to_numeric(
    emissions_long["emissions"].astype(str).str.replace("−", "-", regex=False),
    errors="coerce"
)
...
fig_chart = px.line(
    ...
    labels={"emissions": "Emissions", "year": "Year", "country": "Country"}
)
